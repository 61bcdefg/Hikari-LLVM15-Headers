...
#ifndef LLVM_TRANSFORMS_OBFUSCATION_COMPAT_LEGACYLOWERSWITCH_H
#define LLVM_TRANSFORMS_OBFUSCATION_COMPAT_LEGACYLOWERSWITCH_H
...
class LegacyLowerSwitch : public FunctionPass {
public:
  // Pass identification, replacement for typeid
  static char ID;

  LegacyLowerSwitch() : FunctionPass(ID) {}
...
unsigned LegacyLowerSwitch::Clusterify(CaseVector &Cases, SwitchInst *SI) {
  unsigned numCmps = 0;
...
void LegacyLowerSwitch::processSwitchInst(
  ...
  if (isa<UnreachableInst>(Default->getFirstNonPHIOrDbg())) {
    ...
    UnreachableRanges.emplace_back(R);
